...
//    }
    ...
    public void deleteConversationFromMemory(String conversationId) {
        int index = -1;
        Conversation deletedConverstion;
        for (Conversation tempConversation : conversations) {
        ...
        if (index != -1) {
            deletedConverstion = conversations.get(index);
            conversations.remove(index);
            sortConversationsInMemory();
            notifyConversationRemoved(deletedConverstion, null);
        }

    }


    private Conversation deleteConversationFromMemory2(String conversationId) {
        int index = -1;
        Conversation deletedConverstion = null;
        for (Conversation tempConversation : conversations) {
            if (tempConversation.getConversationId().equals(conversationId)) {
                index = conversations.indexOf(tempConversation);
                break;
        ...
        nodeConversation.removeValue(new DatabaseReference.CompletionListener() {
            ...
            public void onComplete(DatabaseError databaseError, DatabaseReference databaseReference) {
                ...
                if (databaseError == null) {
                    Conversation conversation = deleteConversationFromMemory2(conversationId);
