...
public class ConversationMessagesHandler {
    ...
    private DatabaseReference conversationMessagesNodeRec;
    private DatabaseReference senderConversationNode;
    private DatabaseReference receiverConversationNode;
    ...
    public ConversationMessagesHandler(String firebaseUrl, String appId, IChatUser currentUser, IChatUser recipient) {
        ...
        if (StringUtils.isValid(firebaseUrl)) {
            ...
            this.conversationMessagesNodeRec = FirebaseDatabase.getInstance().getReferenceFromUrl(firebaseUrl).child("/apps/" + appId + "/users/" + recipient.getId() + "/messages/" + currentUser.getId());
            this.senderConversationNode = FirebaseDatabase.getInstance().getReferenceFromUrl(firebaseUrl).child("/apps/" + appId + "/users/" + currentUser.getId() + "/conversations/" + recipient.getId());
            this.receiverConversationNode = FirebaseDatabase.getInstance().getReferenceFromUrl(firebaseUrl).child("/apps/" + appId + "/users/" + recipient.getId() + "/conversations/" + currentUser.getId());
        } else {
            ...
            this.conversationMessagesNodeRec = FirebaseDatabase.getInstance().getReference().child("/apps/" + appId + "/users/" + recipient.getId() + "/messages/" + currentUser.getId());
            this.senderConversationNode = FirebaseDatabase.getInstance().getReference().child("/apps/" + appId + "/users/" + currentUser.getId() + "/conversations/" + recipient.getId());
            this.receiverConversationNode = FirebaseDatabase.getInstance().getReference().child("/apps/" + appId + "/users/" + currentUser.getId() + "/conversations/" + recipient.getId());
        }

        this.conversationMessagesNode.keepSynced(true);
        this.conversationMessagesNodeRec.keepSynced(true);
        this.senderConversationNode.keepSynced(true);
        this.receiverConversationNode.keepSynced(true);
...
//        conversationMessagesListeners.add(conversationMessagesListener);
        ...
        if (conversationMessagesChildEventListener == null) {
            ...
            conversationMessagesNode.addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    if (conversationMessagesListeners != null) {
                        for (ConversationMessagesListener conversationMessagesListener : conversationMessagesListeners) {
                            conversationMessagesListener.onConversationCountRecieved(dataSnapshot.getChildrenCount());
                        }
                    }
                }
                    ...
                    .addChildEventListener(new ChildEventListener() {
                        ...
                        public void onChildAdded(DataSnapshot dataSnapshot, String prevChildKey) {
                            ...
                            try {
                                ...
                                if (conversationMessagesListeners != null) {
                                    for (ConversationMessagesListener conversationMessagesListener : conversationMessagesListeners) {
                                        conversationMessagesListener.onConversationMessageReceived(message, false, null);
                                    }
                                }
                        ...
                        public void onChildRemoved(DataSnapshot dataSnapshot) {
                            Log.v(TAG, "ConversationMessagesHandler.connect.onChildDeleted");

                            try {
                                Message message = decodeMessageSnapShop(dataSnapshot);

                                Log.d(TAG, "ConversationMessagesHandler.connect.onChildDeleted.message : " + message);

                                saveOrUpdateMessageInMemory(message);

                                if (conversationMessagesListeners != null) {
                                    for (ConversationMessagesListener conversationMessagesListener : conversationMessagesListeners) {
                                        conversationMessagesListener.onConversationMessageDeleted(message, null);
                                    }
                                }

                            } catch (ChatFieldNotFoundException cfnfe) {
                                Log.w(TAG, "Error decoding message on onChildDelete " + cfnfe.getMessage());
                            } catch (Exception e) {
                                if (conversationMessagesListeners != null) {
                                    for (ConversationMessagesListener conversationMessagesListener : conversationMessagesListeners) {
                                        conversationMessagesListener.onConversationMessageDeleted(null, new ChatRuntimeException(e));
                                    }
                                }
                            }
                        }
...
//        Log.d(TAG, "message >: " + dataSnapshot.toString());
    ...
    public void removeChildEventListener() {
        conversationMessagesChildEventListener = null;
    }

    public ChildEventListener getChildEventListenerInstance() {
        return conversationMessagesChildEventListener;
    }
    ...
    public void removeSenderMessage(final String messageId, final Message message, boolean deleteFromConversation) {

        conversationMessagesNode.child(messageId).removeValue(new DatabaseReference.CompletionListener() {
            @Override
            public void onComplete(@Nullable DatabaseError databaseError, @NonNull DatabaseReference databaseReference) {
                if (databaseError == null) {


                    conversationMessagesNodeRec.child(messageId).removeValue(new DatabaseReference.CompletionListener() {
                        @Override
                        public void onComplete(@Nullable DatabaseError databaseError, @NonNull DatabaseReference databaseReference) {
                            if (databaseError == null) {
                                if (conversationMessagesListeners != null) {
        ...
        if (deleteFromConversation) {
            senderConversationNode.addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    DatabaseReference numMesasReference = dataSnapshot.getRef().child("last_message_text");
                    numMesasReference.setValue("Message Deleted");
                    senderConversationNode.removeEventListener(this);
                }
            ...
            receiverConversationNode.addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    DatabaseReference numMesasReference = dataSnapshot.getRef().child("last_message_text");
                    numMesasReference.setValue("Message Deleted");
                    receiverConversationNode.removeEventListener(this);
                }
    ...
    public void removeRecieverMessage(String messageId, final Message message, boolean deleteFromConversation) {
        conversationMessagesNode.child(messageId).removeValue(new DatabaseReference.CompletionListener() {
            @Override
            public void onComplete(@Nullable DatabaseError databaseError, @NonNull DatabaseReference databaseReference) {
                if (databaseError == null) {
    ...
    public void removeConversationNode(final ConversationNodeRemoveListener listener) {
        senderConversationNode.removeValue(new DatabaseReference.CompletionListener() {
            @Override
            public void onComplete(@Nullable DatabaseError databaseError, @NonNull DatabaseReference databaseReference) {
                if (databaseError == null) {
                    receiverConversationNode.removeValue(new DatabaseReference.CompletionListener() {
                        @Override
                        public void onComplete(@Nullable DatabaseError databaseError, @NonNull DatabaseReference databaseReference) {
                            if (databaseError == null) {
                                listener.onConversationNodeRemoved(true);
                            } else {
                                listener.onConversationNodeRemoved(false);
                            }
                        }
                    });
                } else {
    ...
    public interface ConversationNodeRemoveListener {
        void onConversationNodeRemoved(boolean isSuccess);
    }
}